<script lang="ts">
	...
	function parseIntoSections(md: string): { title: string; sections: Section[] } {
		...
		const title = h1Match ? h1Match[1] : data.toolType.name;
	...
	function getSectionIcon(id: string): string {
		...
		if (id.includes('evaluation') || id.includes('metrics')) return '🧪';
		return '📝';
	}
</script>
...
<div class="mx-auto flex max-w-[1440px] flex-col gap-8 px-4 py-8 sm:px-8 sm:py-12">
	...
	<nav class="flex flex-wrap items-center gap-2 text-sm text-muted">
		...
		<span>/</span>
		<a href="/tool-type/{data.toolType.key}" class="transition-colors hover:text-black">
			{data.toolType.name}
		</a>
		<span>/</span>
	...
	<div class="flex flex-col gap-4">
		<div class="flex flex-wrap items-center gap-3">
			<span
				class="inline-block rounded-md bg-purple-50 px-2 py-1 text-center text-sm font-semibold text-purple-700"
			>
				{data.toolType.name}
			</span>
		...
		<p class="text-text-secondary">{data.toolType.description}</p>
	</div>
	...
	<div class="rounded-[14px] border border-black/10 bg-surface/50 px-5 py-4">
		<p class="text-sm leading-relaxed text-text-secondary">
			...
			papers (scored ≥7/10) classified under this tool type, extracted key sections (abstract,
			introduction, results, discussion, conclusions) from each, then used Claude to synthesise findings
			into a structured evidence summary. The focus is on what benchmarks and evaluation methods exist
			to measure whether these tools work in the lab.
		</p>
	</div>
	...
	<div class="flex flex-wrap items-center gap-3">
		<a
			href="/tool-type/{data.toolType.key}"
