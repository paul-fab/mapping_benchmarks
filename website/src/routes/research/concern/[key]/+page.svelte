<script lang="ts">
	...
	function parseIntoSections(md: string): { title: string; sections: Section[] } {
		...
		const title = h1Match ? h1Match[1] : data.concern.name;
	...
	function getSectionIcon(id: string): string {
		...
		if (id === 'key-findings') return '🔍';
		if (id.includes('evidence-for')) return '⚠️';
		if (id.includes('mitigating')) return '🛡️';
		if (id.includes('measured') || id.includes('gaps')) return '📊';
		if (id.includes('context')) return '🔄';
		if (id.includes('notable') || id.includes('studies')) return '🔬';
		if (id.includes('lmic')) return '🌍';
		if (id === 'recommendations') return '💡';
		if (id === 'top-papers') return '📄';
		return '📝';
	}
</script>
...
<div class="mx-auto flex max-w-[1440px] flex-col gap-8 px-4 py-8 sm:px-8 sm:py-12">
	...
	<nav class="flex flex-wrap items-center gap-2 text-sm text-muted">
		...
		<span>/</span>
		<a href="/#concerns" class="transition-colors hover:text-black">Concerns</a>
		<span>/</span>
		<span class="text-black">{data.concern.name}</span>
	</nav>
	...
	<div class="flex flex-col gap-4">
		<div class="flex flex-wrap items-center gap-3">
			<span
				class="inline-block rounded-md bg-red-50 px-2 py-1 text-center text-sm font-semibold text-red-700"
			>
				Concern
			</span>
		...
		<p class="text-text-secondary">{data.concern.description}</p>
	</div>

	<!-- Process explanation -->
	<div class="rounded-[14px] border border-black/10 bg-surface/50 px-5 py-4">
		<p class="text-sm leading-relaxed text-text-secondary">
			<span class="font-medium text-black">How this was produced:</span> We searched our corpus of
			high-relevance papers (scored ≥7/10) for keyword matches related to this concern theme, extracted
			key sections from each matched paper, then used Claude to synthesise what the literature says about
			this risk — including evidence for and against, gaps in measurement, and recommendations.
		</p>
	</div>
